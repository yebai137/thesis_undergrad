...
td_void *sample_comm_vdec_send_stream(td_void *args)
{
    ...
    printf("\033[0;35m chn %d send stream thread return ...  \033[0;39m\n", thread_param->chn_id);
    (td_void)fflush(stdout);
    if (buf != TD_NULL) {
        free(buf);
        buf = TD_NULL;
...
td_s32 sample_comm_vdec_start(td_s32 chn_num, sample_vdec_attr *sample_vdec, td_u32 arr_len)
{
    ...
    for (i = 0; (i < chn_num) && (i < (td_s32)arr_len); i++) {
        ...
        if (sample_vdec[i].type == OT_PT_H264 || sample_vdec[i].type == OT_PT_H265 ||
            ...
            printf("vdec chn %d default param before set: dec_mode=%d out_order=%d video_format=%d compress=%d display_frame_num=%u\n",
                i, chn_param.video_param.dec_mode, chn_param.video_param.out_order,
                chn_param.video_param.video_format, chn_param.video_param.compress_mode, chn_param.display_frame_num);
        ...
        check_chn_return(ss_mpi_vdec_set_display_mode(i, sample_vdec[i].display_mode), i,
