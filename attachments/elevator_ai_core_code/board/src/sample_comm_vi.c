...
#define SYS_CLK_BASE_REG 0x11010000
...
static sample_sns_type g_sns_type[SAMPLE_SENSOR_MAX_NUM] = {
...
void sample_comm_vi_set_sns_type(sample_sns_type sns_type, td_u32 index)
{
    if (index >= SAMPLE_SENSOR_MAX_NUM) {
        return;
    }

    g_sns_type[index] = sns_type;
}

sample_sns_type sample_comm_vi_get_sns_type(td_u32 index)
{
    if (index >= SAMPLE_SENSOR_MAX_NUM) {
      return SONY_IMX347_SLAVE_MIPI_4M_30FPS_12BIT;
    }

    return g_sns_type[index];
}

td_s32 sample_sensor_clock(sample_sns_type sns_type, int index){

    unsigned int clock = 0x0;
    int offset = SYS_CLK_BASE_REG + 0x8440 + index * (0x20);

    if(sns_type == OV_OS04A10_MIPI_4M_30FPS_12BIT || sns_type == OV_OS04A10_MIPI_4M_30FPS_12BIT_WDR2TO1 || sns_type == OV_OS04A10_SLAVE_MIPI_4M_30FPS_12BIT){
        clock = 0x4001;
    }else if(sns_type == OV_OS08A20_MIPI_8M_30FPS_12BIT || sns_type == OV_OS08A20_MIPI_8M_30FPS_10BIT_WDR2TO1){
        clock = 0x4001;
    }else if(sns_type == SC450AI_MIPI_4M_30FPS_10BIT || sns_type == SC450AI_MIPI_4M_30FPS_10BIT_WDR2TO1){
        clock = 0xA001;
    }else if(sns_type == SONY_IMX347_SLAVE_MIPI_4M_30FPS_12BIT){
        clock = 0x8001;
    }else{
        clock = 0x0;
        return TD_FALSE;
    ...
    sample_sys_set_reg(offset, clock);
    return TD_SUCCESS;
}
    ...
    td_bool is_master_pipe)
{
    ...
    if (pipe_info->psfm_attr.psfm_en == TD_TRUE) {
        ret = ss_mpi_vi_set_pipe_psfm_attr(vi_pipe, &pipe_info->psfm_attr);
        if (ret != TD_SUCCESS) {
            sample_print("vi set pipe(%d) psfm attr failed with %#x!\n", vi_pipe, ret);
            goto start_pipe_failed;
        }
    }
