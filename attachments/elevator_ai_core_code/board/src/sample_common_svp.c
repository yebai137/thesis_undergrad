...
static td_bool g_sample_svp_check_init_manages_vb = TD_FALSE;
...
static td_s32 sample_common_svp_check_sys_init_impl(td_void)
{
    td_s32 ret;
    ot_vb_cfg vb_cfg;
    ...
    ret = ss_mpi_vb_exit();
    if (ret == OT_ERR_VB_NOT_PERM) {
        g_sample_svp_check_init_manages_vb = TD_FALSE;
        sample_svp_trace_warning("vb exit not permitted, reuse existing vb state during acl init\n");
    ...
    g_sample_svp_check_init_manages_vb = TD_TRUE;
    return TD_SUCCESS;
}

static td_s32 sample_common_svp_check_sys_exit_impl(td_void)
{
    td_s32 ret;
    ...
    if (g_sample_svp_check_init_manages_vb == TD_FALSE) {
        return TD_SUCCESS;
    }
...
td_s32 sample_common_svp_check_sys_init(td_void)
{
    if (g_sample_svp_init_flag == TD_FALSE) {
        if (sample_common_svp_check_sys_init_impl() != TD_SUCCESS) {
...
td_void sample_common_svp_check_sys_exit(td_void)
{
    ...
    if (g_sample_svp_init_flag == TD_TRUE) {
        ret = sample_common_svp_check_sys_exit_impl();
        if (ret != TD_SUCCESS) {
...
static ot_pixel_format sample_common_svp_get_media_pixel_format(const ot_sample_svp_media_cfg *media_cfg, td_u32 idx)
{
    if (media_cfg == TD_NULL || idx >= OT_SVP_MAX_VPSS_CHN_NUM) {
        return OT_PIXEL_FORMAT_YVU_SEMIPLANAR_420;
    }

    if (media_cfg->pixel_format[idx] == 0) {
        return OT_PIXEL_FORMAT_YVU_SEMIPLANAR_420;
    }

    return media_cfg->pixel_format[idx];
}

/* function : Start Vpss */
static td_s32 sample_common_svp_start_media_vpss(td_s32 vpss_grp_cnt, const ot_sample_svp_media_cfg *media_cfg,
    td_bool multi_frame_mode)
{
    sample_vpss_chn_attr vpss_chn_attr = {0};
    ot_vpss_grp_attr vpss_grp_attr;
    ot_vpss_grp vpss_grp;
    ...
    (td_void)memset_s(&vpss_grp_attr, sizeof(ot_vpss_grp_attr), 0, sizeof(ot_vpss_grp_attr));
    sample_comm_vpss_get_default_grp_attr(&vpss_grp_attr);
    vpss_grp_attr.max_width = media_cfg->pic_size[0].width;
    vpss_grp_attr.max_height = media_cfg->pic_size[0].height;
    vpss_grp_attr.pixel_format = OT_PIXEL_FORMAT_YUV_SEMIPLANAR_420;

    for (td_u32 i = 0; i < media_cfg->chn_num; i++) {
        sample_comm_vpss_get_default_chn_attr(&vpss_chn_attr.chn_attr[i]);
        vpss_chn_attr.chn_attr[i].width = media_cfg->pic_size[i].width;
        vpss_chn_attr.chn_attr[i].height = media_cfg->pic_size[i].height;
        vpss_chn_attr.chn_attr[i].depth = 1;
        vpss_chn_attr.chn_attr[i].pixel_format = sample_common_svp_get_media_pixel_format(media_cfg, i);
        vpss_chn_attr.chn_attr[i].compress_mode = OT_COMPRESS_MODE_NONE;
        vpss_chn_attr.chn_enable[i] = TD_TRUE;

        if (i == 0 && multi_frame_mode == TD_FALSE) {
            continue;
        }

        vpss_grp_attr.max_width = vpss_grp_attr.max_width > media_cfg->pic_size[i].width ?
            vpss_grp_attr.max_width : media_cfg->pic_size[i].width;
        vpss_grp_attr.max_height = vpss_grp_attr.max_height > media_cfg->pic_size[i].height ?
            vpss_grp_attr.max_height : media_cfg->pic_size[i].height;
    }
    vpss_chn_attr.chn_array_size = OT_VPSS_MAX_CHN_NUM;
    if (media_cfg->chn_num > 0) {
        vpss_chn_attr.chn_enable[0] = TD_TRUE;
    }
    if (media_cfg->chn_num > 1) {
        vpss_chn_attr.chn_enable[1] = TD_TRUE;
    }
...
td_s32 sample_common_svp_start_vo(sample_vo_cfg *vo_cfg)
{
	...
	if ((vo_cfg->vo_intf_type & OT_VO_INTF_MIPI) || (vo_cfg->vo_intf_type & OT_VO_INTF_MIPI_SLAVE)) {
    ...
    ot_sample_svp_switch *switch_ptr, ot_pic_size *ext_pic_size_type)
{
    ...
    td_s32 ret = TD_FAILURE;
    sample_sns_type sns_type = sample_comm_vi_get_sns_type(0);
...
static td_s32 sample_common_svp_start_vdec(td_u32 chn_num, sample_vdec_attr *vdec_attr, td_u32 arr_len)
{
    ...
    ret = ss_mpi_vdec_set_display_mode(SAMPLE_SVP_VDEC_CHN_0, vdec_attr[0].display_mode);
    sample_svp_check_exps_goto(ret != TD_SUCCESS, end_1, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error(%#x), sample_comm_vdec_start failed!\n", ret);
...
static td_s32 sample_common_svp_create_vb(ot_sample_svp_media_cfg *media_cfg)
{
    ...
    for (i = 0; i < media_cfg->chn_num; i++) {
        ...
        pic_buf_attr.align = OT_DEFAULT_ALIGN;
        pic_buf_attr.bit_width = OT_DATA_BIT_WIDTH_8;
        pic_buf_attr.pixel_format = sample_common_svp_get_media_pixel_format(media_cfg, i);
        pic_buf_attr.video_format = OT_VIDEO_FORMAT_LINEAR;
    ...
    pthread_t *vdec_thread, ot_sample_svp_media_cfg *media_cfg, sample_vo_cfg *vo_cfg)
{
    ...
    if (media_cfg->chn_num > SAMPLE_SVP_SINGLE_FRAME_VPSS_CHN_NUM && vdec_attr->sample_vdec_video.ref_frame_num >
        ...
        ret = sample_common_svp_start_media_vpss(vpss_grp_cnt, media_cfg, TD_TRUE);
    } else {
        ret = sample_common_svp_start_media_vpss(vpss_grp_cnt, media_cfg, TD_FALSE);
    }
    sample_svp_check_exps_goto(ret != TD_SUCCESS, end_init_2, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error(%#x),sample_common_svp_start_vpss failed!\n", ret);
 ...
 * function : Start Vi/Vpss/Venc/Vo
 */
td_s32 sample_common_svp_npu_start_vi_vpss_venc_vo(sample_vi_cfg *vi_cfg, ot_sample_svp_media_cfg *media_cfg,
    sample_vo_cfg *vo_cfg)
{
    const td_s32 vpss_grp_cnt = 1;
    sample_sns_type sns_type = sample_comm_vi_get_sns_type(0);
