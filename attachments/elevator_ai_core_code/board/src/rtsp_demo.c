...
uint32_t g_rtsp_enable = 0;
rtsp_demo_handle g_rtsplive = NULL;
rtsp_session_handle session_h264 = NULL;
rtsp_session_handle session_h265 = NULL;
...
{
	char *cmd = "ip address show eth0 | grep 'inet' | awk '{print $2}' | cut -d '/' -f1";
	FILE *fp = NULL;
...
rtsp_session_handle rtsp_new_session (rtsp_demo_handle demo, const char *path)
{
	...
	get_ip_addr(ip_addr);
	ip_addr[strlen(ip_addr) - 1] = '\0'; //remove '\n'
	// dbg("rtsp://%s:%d%s\n", ip_addr, g_port, s->path);
...
rtsp_session_handle create_rtsp_session(rtsp_demo_handle demo, const enum rtsp_codec_id id, const char *path)
{
    rtsp_session_handle session;
    session = rtsp_new_session(demo,path);

    rtsp_set_video(session, id, NULL, 0);
